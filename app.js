...
citySearchButton.addEventListener("click",() => {
 ...
 function getCurrentLocationweather(lat,lon) {
    ...
    fetch(`http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&lang=pt_br&appid=${api_key}`)
 ...
 function getCityWeather(cityName) {
    ...
    fetch(`http://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=metric&lang=pt_br&appid=${api_key}`)
    .then((response) => response.json())
    .then((data) => displayWeather(data))
 }
